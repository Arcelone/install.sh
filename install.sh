...
main() {
    ...
    if available apt-get && apt_supported; then
        ...
        show printf "%s\n" \
            "Types: deb" \
            "URIs: https://brave-browser-apt-${CHANNEL}.s3.brave.com" \
            "Signed-By: /usr/share/keyrings/brave-browser${CHANNELdash}-archive-keyring.gpg" \
            "Architectures: amd64 arm64" \
            "Suites: stable" \
            "Components: main"|\
                show $sudo install -DTm644 /dev/stdin /etc/apt/sources.list.d/brave-browser-${CHANNEL}.sources
        show $sudo rm -f /etc/apt/sources.list.d/brave-browser-${CHANNEL}.list
        show $sudo apt-get update
